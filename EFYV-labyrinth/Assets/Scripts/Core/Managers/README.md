...
| [`SpawnManager.cs`](SpawnManager.cs) | Difficulty curve, bounded spawning, enemy/prop central ticks, scene-placed entity promotion, survival-time achievement feed; every frame's spawn angles use one native paired-sin/cos batch, and `Start` prewarms the enemy prefab pools |
