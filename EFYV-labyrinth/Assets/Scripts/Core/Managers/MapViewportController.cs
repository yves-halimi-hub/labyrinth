...
namespace EFYV.Core.Managers
{
    ...
    public class MapViewportController : MonoBehaviour
    {
        ...
        // Item #5: imported maps (EFYVMapImporter output). LoadMapData
        // resolves the requested id here first (then against any loaded
        // MapAssetData) and only regenerates procedural noise when no
        // imported map matches.
        [Tooltip(GameConfig.Map.TooltipImportedMaps)]
        public EFYV.Core.Data.MapAssetData[] importedMaps;
        ...
        // Item #5: loads IMPORTED map data (binary .efyvmap files published by
        // LabyMake and ingested by EFYVMapImporter into MapAssetData assets).
        // Procedural generation stays as the EXPLICIT fallback when no
        // imported map matches the requested id.
        public void LoadMapData(string mapId)
        {
            EFYV.Core.Data.MapAssetData imported = FindImportedMap(mapId);
            if (imported != null)
            {
                if (backendMap == null ||
                    backendMap.Width != imported.width ||
                    backendMap.Height != imported.height)
                {
                    backendMap = new FastGridMap(imported.width, imported.height);
                }
                imported.CopyTilesTo(backendMap);
                // tilePalette follows the imported tileset when one resolved
                // at import time; slice i is FastGridMap tile id i.
                if (imported.tileset != null &&
                    imported.tileset.tileSprites != null &&
                    imported.tileset.tileSprites.Length > GameConfig.Runtime.EmptyCollectionCount)
                {
                    tilePalette = imported.tileset.tileSprites;
                }
                Debug.LogFormat(GameConfig.Map.LogImportedMapLoaded, mapId, imported.width, imported.height);
            }
            else
            {
                Debug.LogFormat(GameConfig.Map.LogNoImportedMapFallback, mapId);
                if (backendMap == null)
                    backendMap = new FastGridMap(GameConfig.Map.DefaultMapWidth, GameConfig.Map.DefaultMapHeight);
        ...
        // Resolves an imported map by id: the serialized importedMaps slots
        // first, then any loaded MapAssetData (editor/play-mode convenience -
        // freshly imported assets appear without hand-wiring the array).
        // Entries whose tile payload cannot back a grid are skipped.
        internal EFYV.Core.Data.MapAssetData FindImportedMap(string mapId)
        {
            if (string.IsNullOrEmpty(mapId)) return null;
            if (importedMaps != null)
            {
                for (int index = GameConfig.Runtime.FirstIndex; index < importedMaps.Length; index++)
                {
                    EFYV.Core.Data.MapAssetData candidate = importedMaps[index];
                    if (candidate != null && candidate.HasLoadableTiles() &&
                        string.Equals(candidate.mapId, mapId, System.StringComparison.Ordinal))
                        return candidate;
                }
            }

            EFYV.Core.Data.MapAssetData[] loaded =
                Resources.FindObjectsOfTypeAll<EFYV.Core.Data.MapAssetData>();
            for (int index = GameConfig.Runtime.FirstIndex; index < loaded.Length; index++)
            {
                EFYV.Core.Data.MapAssetData candidate = loaded[index];
                if (candidate != null && candidate.HasLoadableTiles() &&
                    string.Equals(candidate.mapId, mapId, System.StringComparison.Ordinal))
                    return candidate;
            }
